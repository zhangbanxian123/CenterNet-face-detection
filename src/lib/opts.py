...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--dataset', default='meeting',
                             help='coco | kitti | coco_hp | pascal| meeting')
    ...
    self.parser.add_argument('--demo', default='../images/',   #默认检测images文件夹下所有图片
                             help='path to image/ image folders/ video. '
                                  'or "webcam"')
    self.parser.add_argument('--load_model', default='../exp/ctdet/coco_dla/model_last.pth', #默认使用最优模型
    ...
    self.parser.add_argument('--vis_thresh', type=float, default=0.3,  #预测类别的阈值
    ...
    self.parser.add_argument('--num_epochs', type=int, default=400,  #迭代次数
    ...
    self.parser.add_argument('--flip_test', action='store_true',
                             help='flip data augmentation.')  #翻转数据扩充
    ...
    self.parser.add_argument('--not_reg_offset', action='store_true',
                             help='not regress local offset.') #不回归本地偏移
    # exdet
    self.parser.add_argument('--agnostic_ex', action='store_true',
                             help='use category agnostic extreme points.')
    self.parser.add_argument('--scores_thresh', type=float, default=0.1,   #热力图阈值
                             help='threshold for extreme point heatmap.')
    self.parser.add_argument('--center_thresh', type=float, default=0.1,  #中心点的阈值
  ...
  def init(self, args=''):
    default_dataset_info = {
      'ctdet': {'default_resolution': [512, 512], 'num_classes': 3,   #针对自己的数据集
                'mean': [2.133921, 2.271873, 2.351718], 'std': [5.289286, 5.630496, 5.829290],
                'dataset': 'meeting'},
      # 'ctdet': {'default_resolution': [512, 512], 'num_classes': 80,  #使用原始模型测试
