...
class Debugger(object):
  def __init__(self, ipynb=False, theme='black', 
    ...
    self.ipynb = ipynb
    # print('self.ipynb', self.ipynb) # Flase
    ...
    elif num_classes == 3 or dataset == 'meeting':
      self.names = meeting_class_name
  ...
  def show_img(self, pause = False, imgId = 'default'):
    cv2.imwrite('{}'.format(imgId), self.imgs[imgId])
    if pause:
      cv2.waitKey()
  ...
  def add_coco_bbox(self, emotion, bbox, cat, conf=1, show_txt=True, img_id='default'): 
    ...
    txt = '{}-{}'.format(self.names[cat], emotion)
  ...
  def save_img(self, imgId='default', path='../output'):
  ...
  def show_imgs(self):
    for i, v in self.imgs.items():
      ...
      cv2.waitKey(1)
      # cv2.destroyAllWindows()
  def save_all_imgs(self, path='../output', prefix='', genID=False):
    if genID:
      try:
    ...
    m=0
    for i, v in self.imgs.items():  #测试发现只有一轮
      m +=1
...
meeting_class_name = ['down','up','rotate']
