...
class BaseDetector(object):
  ...
  def emotion_test(self, img):
    # img = cv2.imread(img)
    # img = cv2.resize(img,(224,224))
    img = trans(img)
    img =img.to(self.opt.device)
    img = img.unsqueeze(0)
    output = self.model_emotion(img)
  ...
  def run(self, image_or_path_or_tensor, out=None, meta=None):
    ...
    if self.opt.debug >= 1:
      concentrate_rate, activity_rate = self.show_results(debugger, image, results, out=out)
